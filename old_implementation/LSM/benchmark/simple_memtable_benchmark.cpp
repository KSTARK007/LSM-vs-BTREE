...
#include "../memtable/custom_skiplist.h"
...
std::string results_FILE = "simple_memtable_benchmark.csv";
...
void worker_get(const std::vector<std::pair<uint64_t, char>>& ops, int start, int end, int thread_id, 
                SkipList<std::string, std::string>* skiplist, std::vector<double>& local_read_latencies, 
    ...
    if (results_FILE == "a.csv") {
        ...
        write_ratio = 0.0; // Read-only for memtable testing
    }
    ...
    while (elapsed_seconds.count() < total_runtime) {
        for (int i = start; i < end; ++i) {
            ...
            op = zipf.get_op();
            // For reads, use keys that are more likely to exist in the skiplist
            if (op == 'R') {
                // Use keys from the existing data (0 to skiplist size)
                uint64_t key_index = zipf.Next() % skiplist->size();
                key = generate_ycsb_like_key(key_index);
                
                t1 = __rdtscp(&tsc_aux);
                std::string* found = skiplist->find_wait_free(key);
                t2 = __rdtscp(&tsc_aux);
                if (!found) {
                    local_failures++;
                }
                ...
                // For updates, use existing keys to avoid skiplist overflow
                uint64_t key_index = zipf.Next() % skiplist->size();
                key = generate_ycsb_like_key(key_index);
                
                t1 = __rdtscp(&tsc_aux);
                skiplist->insert(key, val_to_insert); // This will be an update
                t2 = __rdtscp(&tsc_aux);
                auto duration = cycles_to_nanoseconds(t2 - t1, CPU_FREQ_GHZ);
...
void benchmark(int num_threads, const std::vector<std::pair<uint64_t, std::string>>& data,
               ...
               CSVLogger& pagenumbers, SkipList<std::string, std::string>* skiplist) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        threads.emplace_back([&, i, beg, end]() {
            ...
            std::vector<double> local_write_latencies;
            worker_get(ops, beg, end, i, skiplist, local_read_latencies, local_write_latencies);
...
int main(int argc, char* argv[]) {
    ...
    try {
        // Create skiplist directly
        auto skiplist = std::make_unique<SkipList<std::string, std::string>>();
        
        std::cout << "Creating Custom SkipList and inserting YCSB-style data...\n";
        ...
        for (uint64_t i = 0; i < TOTAL_KEYS; i++) {
            ...
            skiplist->insert(ycsb_key, random_value);
        ...
        std::cout << "Inserted " << skiplist->size() << " YCSB-style key/value pairs.\n";
        ...
        if (argc > 1) {
            ...
            benchmark(Number_of_threads, dummy_data, ops, logger, pagenumbers, skiplist.get());
        } else {
            std::cout << "No thread number provided. Running with 1 thread.\n";
            benchmark(1, dummy_data, ops, logger, pagenumbers, skiplist.get());
        }
